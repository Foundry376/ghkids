 ...
 *   --bucket NAME     Supabase bucket (default: background-images).
...
const parseArgs = () => {
  ...
  const out: { provider?: string; dir?: string; bucket: string; dryRun: boolean } = {
...
const main = async () => {
  ...
  const missing = manifest.images.filter((img) => !fs.existsSync(path.join(absDir, img.file)));
  if (missing.length > 0) {
  ...
  const uploads: { localPath: string; remotePath: string; contentType: string }[] = [];
  ...
  for (const u of uploads) {
    ...
    const { error } = await supabase.storage
      .from(bucket)
      .upload(u.remotePath, buf, { contentType: u.contentType, upsert: true });
    if (error) {
